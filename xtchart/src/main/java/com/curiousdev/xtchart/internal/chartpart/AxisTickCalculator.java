...
public abstract class AxisTickCalculator {
   ...
   * @param styleManager
   */
  public AxisTickCalculator(Axis.Direction axisDirection, double workingSpace, double minValue, double maxValue, StyleManager styleManager) {

    // override min/max value for bar charts' Y-Axis
    double overrideMinValue = minValue;
    double overrideMaxValue = maxValue;
    if (styleManager.getChartType() == ChartType.Bar && axisDirection == Axis.Direction.Y) { // this is the Y-Axis for a bar chart
    ...
    if (axisDirection == Axis.Direction.X && styleManager.getXAxisMin() != null && styleManager.getChartType() != ChartType.Bar) {
      overrideMinValue = styleManager.getXAxisMin();
    }
    if (axisDirection == Axis.Direction.Y && styleManager.getYAxisMin() != null) {
      overrideMinValue = styleManager.getYAxisMin();
    }
    if (axisDirection == Axis.Direction.X && styleManager.getXAxisMax() != null && styleManager.getChartType() != ChartType.Bar) {
      overrideMaxValue = styleManager.getXAxisMax();
    }
    if (axisDirection == Axis.Direction.Y && styleManager.getYAxisMax() != null) {
  ...
  public double getNumericalGridStep(double tickSpace) {
    ...
    int tickMarkSpaceHint = (axisDirection == Axis.Direction.X ? styleManager.getXAxisTickMarkSpacingHint() : styleManager.getYAxisTickMarkSpacingHint());

    // for very short plots, squeeze some more ticks in than normal
    if (axisDirection == Axis.Direction.Y && tickSpace < 160) {
