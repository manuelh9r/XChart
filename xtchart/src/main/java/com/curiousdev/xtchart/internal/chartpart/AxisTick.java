...
public class AxisTick implements ChartPart {
  ...
  public void paint(Graphics2D g) {
    ...
    if (axis.getDirection() == Axis.Direction.X && getChartPainter().getStyleManager().getChartType() == StyleManager.ChartType.Bar) {

      axisTickCalculator = new AxisTickBarChartCalculator(axis.getDirection(), workingSpace, axis.getMin(), axis.getMax(), getChartPainter());

    }
    else if (axis.getDirection() == Axis.Direction.X && getChartPainter().getStyleManager().isXAxisLogarithmic() && axis.getAxisType() != Axis.AxisType.Date) {
    ...
    else if (axis.getDirection() == Axis.Direction.Y && getChartPainter().getStyleManager().isYAxisLogarithmic() && axis.getAxisType() != Axis.AxisType.Date) {

      axisTickCalculator = new AxisTickLogarithmicCalculator(axis.getDirection(), workingSpace, axis.getMin(), axis.getMax(), getChartPainter().getStyleManager());

    }
    else if (axis.getAxisType() == Axis.AxisType.Number) {
    ...
    else if (axis.getAxisType() == Axis.AxisType.Date) {
