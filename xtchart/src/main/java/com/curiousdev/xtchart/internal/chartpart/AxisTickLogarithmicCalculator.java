...
public class AxisTickLogarithmicCalculator extends AxisTickCalculator {
   ...
   * @param styleManager
   */
  public AxisTickLogarithmicCalculator(Axis.Direction axisDirection, double workingSpace, double minValue, double maxValue, StyleManager styleManager) {
  ...
  private void calculate() {
    ...
    if (axisDirection == Axis.Direction.X && styleManager.getXAxisMin() != null) {
      logMin = (int) (Math.log10(styleManager.getXAxisMin())); // no floor
    }
    if (axisDirection == Axis.Direction.X && styleManager.getXAxisMax() != null) {
