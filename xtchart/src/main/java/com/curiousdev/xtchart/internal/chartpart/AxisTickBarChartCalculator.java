...
public class AxisTickBarChartCalculator extends AxisTickCalculator {
   ...
   * @param maxValue
   */
  public AxisTickBarChartCalculator(Axis.Direction axisDirection, double workingSpace, double minValue, double maxValue, ChartPainter chart) {
  ...
  private void calculate(ChartPainter chartPainter) {
    ...
    for (Series series : chartPainter.getAxisPair().getSeriesMap().values()) {
      ...
      TDoubleIterator firstSeriesItr = firstSeries.getXData().iterator();
      TDoubleIterator xItr = series.getXData().iterator();
      while (xItr.hasNext()) {
